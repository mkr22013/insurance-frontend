...
export default function App({
  ...
  useEffect(() => {
    // Only load welcome when authenticated or activeMemberKey is set
    // Unauthenticated: show basic message, wait for "Use Demo Member" button
  ...
  const loadDemoMember = async () => {
    try {
      ...
      setActiveMemberKey("DEMO000001"); // triggers useEffect → loadWelcome
    } catch {
      setMessages((prev) => [
        ...prev,
                  ...
                  (msg.pages && msg.pages?.length > 0 ||
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-100 flex items-start gap-2">
                      ...
                      <div className="text-[12px] text-slate-500 leading-snug">
                        ...
                        {msg.source.includes("||") ? (
                          // Rx dual-source: "Booklet A | Pages X, Y || Booklet B | Page Z"
                          <span>
                            {msg.source.split("||").map((part, i) => (
                              <span key={i}>
                                {i > 0 && (
